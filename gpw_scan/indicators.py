...
def ohlcv_to_weekly(df_d: pd.DataFrame) -> pd.DataFrame:
    """
    Weekly poglądowo: W-FRI.
    Odcina niedomknięty tydzień.
    """
    w = pd.DataFrame({
    ...
    if not w.empty:
        ...
        last_week_label   = w.index.max().normalize()
        if last_session_date < last_week_label:
            w = w.iloc[:-1]
    return w

def return_nd(close: pd.Series, n: int):
